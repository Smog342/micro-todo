...
function PlannedComponent(){
    ...
    return(
        <div className="pl-40 overflow-auto h-full">
            ...
            {tasks.map((task : Task) => (task.date !== '' ? <TaskComponent id={task.id} text={task.text} date={task.date} important={task.important} finished={task.finished} board={task.board} key={task.id}></TaskComponent> : <></>))}
        </div>
    )
